...
ACTIVATE MSDIALOG oDlg CENTER ON INIT MyEnchoBar(oDlg,{||Iif(MsgYesNo("Deseja sair ?","Confirmação"),oDlg:End(),.F.)})
RETURN
...
If SubStr(cVersao,1,3)<>"MP8"
   DEFINE BUTTON oBtnHlp RESOURCE "S4WB016N"  OF oBar ACTION MsgInfo("Primeiro botão",FunName()) TOOLTIP "Help do programa..."
   DEFINE BUTTON oBtnPsq RESOURCE "LOCALIZA"  OF oBar ACTION MsgInfo("Segundo botão",FunName())  TOOLTIP "Localizar..."
   DEFINE BUTTON oBtnUsr RESOURCE "USER"      OF oBar ACTION MsgInfo("Terceiro botão",FunName()) TOOLTIP "Listar todos..."
   DEFINE BUTTON oBtnImp RESOURCE "IMPRESSAO" OF oBar ACTION MsgInfo("Quarto botão",FunName())   TOOLTIP "Imprimir Doc..."
   DEFINE BUTTON oBtnNp  RESOURCE "PRINT02"   OF oBar ACTION MsgInfo("Quinto botão",FunName())   TOOLTIP "Imprimir NF..."
